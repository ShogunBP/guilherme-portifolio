...
PROJECT_DIR="/srv/minecraft/tb-portifolio"
COMPOSE_FILE="api/docker-compose.yml"
PROJECT_NAME="tb-portifolio"
LAST_DEPLOY_FILE="$PROJECT_DIR/.last-deploy-commit"

if [ ! -f "$COMPOSE_FILE" ]; then
    echo "❌ Erro: api/docker-compose.yml não encontrado"
    exit 1
fi
...
if [ "$FORCE_REBUILD" = true ]; then
    ...
    if [ -z "$LAST_COMMIT" ] || [ "$LAST_COMMIT" != "$CURRENT_COMMIT" ]; then
        ...
        if echo "$CHANGED_FILES" | grep -qE "(src/|api/|Dockerfile|package\.json|vite\.config|docker-compose)"; then
            NEED_REBUILD=true
...
docker compose -p "$PROJECT_NAME" -f "$COMPOSE_FILE" down 2>/dev/null || true
docker rm -f portfolio-api portfolio-frontend tb-api tb-frontend 2>/dev/null || true

# 7. Criar diretórios necessários
echo ""
echo "📁 Verificando volumes..."
mkdir -p "$PROJECT_DIR/api/data" "$PROJECT_DIR/api/uploads" "$PROJECT_DIR/api/logs" "$PROJECT_DIR/api/logs/nginx" || true

# 8. Limpar logs antigos (evita crescimento infinito)
echo ""
echo "📋 Limpando logs antigos (>30 dias)..."
find "$PROJECT_DIR/api/logs" -type f -name "*.log" -mtime +30 -delete 2>/dev/null || true
...
echo "🔍 Backend (API):"
docker logs tb-api 2>&1 | grep -E "(Version|Deploy timestamp|BUILD)" | tail -5 || echo "   (aguardando logs...)"
echo "   Status: $(docker ps --filter name=tb-api --format '{{.Status}}' 2>/dev/null || echo 'N/A')"

echo ""
echo "🔍 Frontend:"
echo "   Status: $(docker ps --filter name=tb-frontend --format '{{.Status}}' 2>/dev/null || echo 'N/A')"
echo "   Abra o console do navegador (F12) para ver o timestamp"

echo ""
echo "📊 Health check..."
sleep 10
API_VERSION=$(curl -s http://localhost:3001/api/health 2>/dev/null | grep -o '"version":"[^"]*"' | cut -d'"' -f4 || echo 'N/A')
API_DEPLOY=$(curl -s http://localhost:3001/api/health 2>/dev/null | grep -o '"deployDate":"[^"]*"' | cut -d'"' -f4 || echo 'N/A')
...
echo "   Nome: $PROJECT_NAME (tb-portifolio)"
echo "   Containers: tb-api, tb-frontend"
echo ""
